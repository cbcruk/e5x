...
- **제거된 원소 retention은 고치지 않고 문서화 (결정)**: memo가 마지막 결과 배열을 다음 읽기까지 들고 있다. 구독
  ...
  같은 이유로 memo의 `cachedDeps`도 붙잡는다: collection을 dep으로 준 뷰는 dep에서 제거된 원소(proxy 배열)를 **뷰 자신**을
  다시 읽을 때까지 붙잡는다. dep만 다시 읽어서는 풀리지 않는다(리뷰어가 찾음, 테스트로 고정).
...
| #16 | #2   | 0 / 4                       | 3 (+1은 머지 전 확인 요청으로 수용)        | 0    | 1         | 리뷰어가 누수를 6가지 더 넣어 테스트 검출력을 확인했고, `cachedDeps`를 통한 retention을 Chromium에서 재현함. 작성자의 첫 재현 테스트는 뷰를 붙잡지 않아 틀렸다가 고침. 재리뷰 생략 |
...
- 구독 없이 붙잡힌 collection은 DOM에서 제거된 원소를 다음 읽기까지 붙잡는다. collection dep의 제거된 원소도
  뷰를 다시 읽을 때까지 붙잡는다 (#2에서 문서화, 수정 보류).
