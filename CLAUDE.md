...
- **제거된 원소 retention 수정 (사용자 결정, 처음엔 문서화로 제안했음)**: memo가 마지막 결과 배열과 dep 값을 다음
  ...
  - 결과는 `Cell`(plain object)에 담아 노드별 `cellsByNode`에 등록하고, `ingest`가 그 노드 version을 올릴 때 비운다.
    closure가 아니라 plain object인 이유: V8 closure는 바깥 스코프 전체를 잡아서, 등록부가 버려진 뷰까지 다음
    mutation 때까지 살려두기 때문. 등록은 계산할 때마다 하고, 비울 때 Set째로 지운다. 그래서 등록부 크기는 마지막
    mutation 이후 계산된 cell 수로 제한된다.
  - dep 값은 `Object.is` 비교용이라 객체면 `WeakRef`로 기억한다. 회수됐다면 지금 값과 같은 객체일 수 없으니 비교는
    올바르게 실패한다.
...
| #16 | #2   | 1회차 0 / 4                 | 1회차 3 (+1은 사용자 확인으로 넘김 → 수정하기로 결정) | 0    | 1         | 리뷰어가 누수를 6가지 더 넣어 테스트 검출력을 확인했고, `cachedDeps`를 통한 retention을 Chromium에서 재현함. 작성자의 첫 재현 테스트는 뷰를 붙잡지 않아 틀렸다가 고침. 리뷰어가 사용자 확인으로 넘긴 결정이 설계 변경으로 이어짐 |
