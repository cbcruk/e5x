...
| #26 | #25  | 1회차 3 / 3, 2회차 1 / 4, 3회차 1 / 4 | 각 회차 전부 반영                                                  | 0    | 3         | 남의 페이지에 도는 코드라 위험이 실제였다: 설치 불가 산출물, `/item` 토론 전체 숨김, `/jobs` 전체 숨김, 그리고 3회차가 잡은 **frozen story**(2회차 수정이 만든 새 버그). 재리뷰 한도(2회)를 다 써서 3회차 must-fix는 작성자 검증으로 머지                                                                                                      |
| #29 | #28  | 1회차 1 / 4, 2회차 0 / 5              | 1회차 5, 2회차 5 (거절 0)                                          | 0    | 2         | 두 회차가 같은 부류를 잡았다: **문서가 세 번 주장한 성질에 테스트가 없음**(1회차 loose 형제, 2회차 `has`의 `noteRead`). 2회차는 `in`이 non-extensible element에서 **throw**하는 것을 프로브로 찾음 — 이 브랜치 전에는 던지지 않던 연산이고 기존 `pinned` 테스트는 불변식의 다른 절반만 밟고 있었다. 변이 16개 주입해 15개가 잡히는 것까지 확인 |
